...
with st.sidebar:
    ...
    # Placeholder fields to avoid pre-filled text [cite: 1, 8, 11, 12]
...
# --- 3. MAIN UI: DOCUMENT CONSTRUCTION ---
...
def clean_text(text):
    if not text: return ""
    # Specifically handling currency and bullet points to prevent '?' errors [cite: 100, 115]
    rep = {
        "’": "'", "‘": "'", "“": '"', "”": '"', "—": "-", "–": "-",
        "€": "EUR ", "$": "USD ", "•": "-" 
    }
...
def create_pdf():
    ...
    for title, content in sections:
        ...
        if cleaned_content and cleaned_content != "VENUE: \nDATE: ":
            # Organization Logic: Start new page if too close to bottom [cite: 117, 119]
            if pdf.get_y() > 230:
                pdf.add_page()
...
if st.button("GENERATE OFFICIAL PDF"):
    if partner and event_name:
        try:
            ...
            st.success(f"Proposal for {partner} created.")
        except Exception as e:
            st.error(f"Execution Error: {e}")
    else:
        st.warning("Please enter a Brand Partner and Activation Title.")
